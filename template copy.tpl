...
function hashFieldsAsync(userData, doneCallback) {
  const maybeFinish = (pending, userDataHashed) => {
    if (pending === 0) doneCallback(userDataHashed);
  };
  let pending = 0;
  ...
  fieldNames.forEach((fieldName) => {
    ...
    const normalized = makeString(normalizeBasedOnSchemaKey(fieldName, value)).toLowerCase().trim();
    sha256(
      normalized,
      (digest) => {
        userDataHashed[fieldName] = digest;
        pending--;
        maybeFinish(pending, userDataHashed);
      },
      () => {
        pending--;
      },
...
function hashFieldsSync(userData) {
  const dataTag256 = copyFromWindow('dataTag256');
  if (getType(dataTag256) !== 'function') return;
  ...
  fieldNames.forEach((fieldName) => {
    ...
    const normalized = makeString(normalizeBasedOnSchemaKey(fieldName, value)).toLowerCase().trim();
    userDataHashed[fieldName] = callInWindow('dataTag256', normalized, 'HEX');
  });
...
function storeEventEnhancement(userData) {
  const storeInLocalStorage = (userData) => {
  ...
  if (localStorage && hasProps(userData)) {
    if (!data.storeUserDataHashed) storeInLocalStorage(userData);
    else {
      const userDataHashed = hashFieldsSync(userData);
      if (hasProps(userDataHashed)) storeInLocalStorage(userDataHashed);
      else {
        const doneCallback = (userDataHashed) => storeInLocalStorage(userDataHashed);
        hashFieldsAsync(userData, doneCallback);
      }
    }
  }
}
